...
class HttpSession @Inject constructor(
    microHttp: MicroHttp,
    val ITempFileManager: ITempFileManager,
        ...
        try {
            ...
            val header = with(String) {
                for (index in 1 until lines.size) {
                    ...
                    if (pos >= 0) {
                        headers[lines[index].substring(0, pos).trim().toLowerCase(Locale.ROOT)] =
    ...
    private fun decodeMultipartFormData(
        ...
        try {
            ...
            for ((index, it) in boundaryIdxs.withIndex()) {
                fbuf.position(it)
                val len =
                    (fbuf.remaining() < MAX_HEADER_SIZE) isTrue fbuf.remaining() ?: MAX_HEADER_SIZE
    ...
    private fun getBoundaryPositions(b: ByteBuffer, boundary: ByteArray): IntArray {
        ...
        //第一次读取
        val firstFill =
            (b.remaining() < searchWindow.size) isTrue b.remaining() ?: searchWindow.size
    ...
    private fun getTmpBucket(): RandomAccessFile {
        return try {
            val tempFile = this.ITempFileManager.createTempFile(null)
            RandomAccessFile(tempFile.getName(), "rw")
        } catch (e: Exception) {
            throw Error(e)
        }
    }
    ...
    //todo kotlin 对文件的扩展函数操作
    private fun saveTmpFile(b: ByteBuffer, offset: Int, len: Int, filename_hint: String?): String {
        var path = ""
        if (len > 0) {

            try {
                val tempFile = ITempFileManager.createTempFile(filename_hint)
                val src = b.duplicate()
                FileOutputStream(tempFile.getName()).use { fileOutputStream ->
